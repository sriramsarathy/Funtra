...
class Tracing {
    ...
    logModulesLoadedBeforeTrace(filesLoadedBeforeTrace) {
        ...
        if (modulesLoadedBeforeTrace.length > 0) {
            this.logger.error('TraceAgent#start: Tracing might not work as the following modules', 'were loaded before the trace agent was initialized:', `[${modulesLoadedBeforeTrace.sort().join(', ')}]`);
        }
    }
    /**
    ...
    enable() {
        ...
        try {
            ...
            this.logger.error('TraceAgent#start: Disabling the Trace Agent for the', `following reason: ${e.message}`);
            this.disable();
            return;
        }
        trace_writer_1.traceWriter.get().initialize((err) => {
            if (err) {
                this.logger.error('TraceAgent#start: Disabling the Trace Agent for the', `following reason: ${err.message}`);
                this.disable();
            }
        });
        ...
        if (typeof this.config.projectId !== 'string' &&
            ...
            this.logger.error('TraceAgent#start: config.projectId, if provided, must be a string.', 'Disabling trace agent.');
            this.disable();
            return;
        }
